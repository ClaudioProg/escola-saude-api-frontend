...
/* ───────────────── Hero padronizado ───────────────── */
function HeaderHero({ variant = "marrom" }) {
  const variants = {
  ...
  const grad = variants[variant] ?? variants.marrom;
    ...
    <header className={`bg-gradient-to-br ${grad} text-white`}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 flex flex-col items-center text-center gap-3">
        <div className="inline-flex items-center gap-2">
          ...
          <h1 className="text-xl sm:text-2xl font-extrabold tracking-tight">Agendamento de Sala</h1>
        </div>
        <p className="text-sm text-white/90">
          Visualize as datas disponíveis e solicite o uso das salas. Sujeito à aprovação da Escola.
        </p>
      </div>
    </header>
  );
