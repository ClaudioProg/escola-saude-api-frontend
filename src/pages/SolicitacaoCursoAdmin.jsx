...
export default function SolicitacaoCursoAdmin() {
  ...
  async function handleExcluir(curso) {
    const confirmar = // ✅ src/pages/SolicitacaoCursoAdmin.jsx — premium (admin)
    // Upgrades: ModalConfirmacao + chips filtros + persistência + calendário premium + KPI por status
        ...
        <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-zinc-950 text-slate-900 dark:text-white">
          ...
          >
            <p className="text-sm text-zinc-700 dark:text-zinc-300">
              ...
              {confirmacao?.titulo ? <> <em className="font-semibold">“{confirmacao.titulo}”</em></> : null}?
            </p>
          </ModalConfirmacao>
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-2 max-w-2xl text-sm text-white/90">
                    Visualize, acompanhe e atualize o status das solicitações de cursos cadastradas por toda a rede de saúde.
                  </p>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {curso.modalidade && (
                            <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 dark:bg-zinc-800 px-2 py-0.5 text-[11px] font-semibold text-slate-700 dark:text-zinc-200">
                              {curso.modalidade === "presencial" ? "Presencial" : curso.modalidade === "online" ? "On-line" : "Híbrido"}
                            </span>
                          )}
      ...
      return (
        <span className={cx("inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-semibold", tones.wrap)}>
          ...
          <button
            type="button"
            onClick={onClear}
            className={cx("ml-1 inline-flex h-4 w-4 items-center justify-center rounded-full", tones.hover, "focus:outline-none focus-visible:ring-2", tones.ring)}
