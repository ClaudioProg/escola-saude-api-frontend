...
function ModalReservaAdmin({
  ...
  // Dados base da data do slot (para defaults de recorrência)
  const baseDate = new Date(`${slot.dataISO}T12:00:00`);
  const baseWeekday = baseDate.getDay(); // 0-6
  ...
  const [ordemSemanaBase, setOrdemSemanaBase] = useState(
    String(baseOrdemSemana)
  ); // '1'..'5'
  ...
  function toggleMes(mIndex) {
    setMesesSelecionados((prev) =>
      prev.includes(mIndex)
        ? prev.filter((m) => m !== mIndex)
        : [...prev, mIndex]
    );
  }
  ...
  function buildRecorrenciaPayload() {
    ...
    if (rep <= 0) {
      toast.warn("Informe a quantidade de repetições da recorrência.");
      return null;
    }

    const baseWeekdayIndex = baseWeekday;
    ...
    if (tipoRecorrencia === "mensal") {
      ...
      // ordem da semana
      const ordem = ehUltimaSemana
        ? baseOrdemSemana
        : Number(ordemSemanaBase) || baseOrdemSemana;
      ...
      return {
        ...
        mensal: {
          modo: "ordem_semana",
          diaSemanaBaseIndex: baseWeekdayIndex,
  ...
  async function salvar() {
    try {
      ...
      if (ehNovo) {
        ...
        if (tipoRecorrencia !== "nenhuma" && !recorrencia) {
          // erro de validação já informado em toast
          return;
        }
        ...
        if (inseridas.length && conflitos.length) {
          toast.success(
            `Criadas ${inseridas.length} reservas. ${conflitos.length} datas já estavam ocupadas.`
          );
        ...
        await api.put(`/salas/admin/reservas/${reserva.id}`, payload);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm">
      ...
      >
        ...
        <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
          ...
          {/* Recorrência (apenas para nova reserva) */}
          {ehNovo && (
            <div className="mt-2 rounded-2xl border border-emerald-100 bg-emerald-50/60 px-3 py-3">
                ...
                <div className="mt-2 space-y-2 text-[11px] text-slate-600">
                  <div className="flex flex-wrap gap-2 items-center">
                    ...
                    <label className="inline-flex items-center gap-1">
                      ...
                      <input
                        ...
                        max={31}
                        value={diaMesBase}
                        onChange={(e) => setDiaMesBase(e.target.value)}
                  ...
                  <div className="flex flex-wrap gap-2 items-center">
                    <label className="inline-flex items-center gap-1">
                      ...
                      na
                      <select
                        disabled={ehUltimaSemana}
                        value={ordemSemanaBase}
                    ...
                    <label className="inline-flex items-center gap-1">
                      <input
                        type="checkbox"
                        checked={ehUltimaSemana}
                        onChange={(e) => setEhUltimaSemana(e.target.checked)}
                      />
              ...
              {tipoRecorrencia !== "nenhuma" && (
                <div className="mt-2 flex items-start gap-2 text-[11px] text-emerald-800">
                  ...
                  <p>
                    A data base é <strong>{slot.dataISO}</strong>. A recorrência
                    será criada somente em dias úteis (sem fins de semana,
                    feriados ou pontos facultativos). Conflitos serão ignorados
                    e informados após o salvamento.
                  </p>
                </div>
              )}
            </div>
