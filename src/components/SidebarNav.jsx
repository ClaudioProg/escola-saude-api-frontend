...
function getPerfisRobusto() {
  ...
  try {
    ...
    if (rawPerfil) {
      try {
        pushAll(out, JSON.parse(rawPerfil));
      } catch {
...
function IconTile({ active, isDark, Icon }) {
  return (
    <span
      className={cx(
        "inline-flex h-9 w-9 items-center justify-center rounded-xl border transition-all duration-200 shrink-0",
        active
          ? "border-white/15 bg-white/10 shadow-[0_4px_20px_rgba(255,255,255,.08)]"
          : isDark
...
function MenuItem({
  ...
  return (
    <button
      ...
      tabIndex={tabIndex}
      className={cx(
        "w-full text-left group relative flex items-center rounded-2xl px-3 py-2",
        "gap-2.5 text-[13px] font-extrabold transition-all duration-200",
        "focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/70",
        active
          ? "text-white bg-emerald-600 shadow-sm shadow-emerald-900/20"
          : isDark
          ? "text-zinc-200 hover:bg-white/5"
          : "text-slate-900 hover:bg-emerald-50/75 hover:shadow-[0_1px_0_rgba(15,23,42,0.04)]",
...
/* =========================
   SidebarNav
========================= */
const SIDEBAR_COLLAPSED_KEY = "escola_sidebar_collapsed";
...
export default function SidebarNav({
  ...
  const isDark = useIsDarkReactive();
  ...
  const menusGestor = useMemo(
    () => [
      { label: "Gestão de Informações", path: "/gestao-informacoes", icon: Megaphone },
