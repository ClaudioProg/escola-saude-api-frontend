...
export default function Navbar() {
  ...
  const [perfil, setPerfil] = useState(() => {
    const armazenado = localStorage.getItem("perfil");
    if (armazenado) {
      return armazenado.split(",").map((p) => p.trim().toLowerCase());
    }
    return ["usuario"];
  });
  ...
  useEffect(() => {
    try {
      const armazenado = localStorage.getItem("perfil");
      if (armazenado) {
        const perfilArray = armazenado.split(",").map((p) => p.trim().toLowerCase());
        setPerfil(perfilArray);
      } else {
        setPerfil(["usuario"]);
      }
    } catch (e) {
  ...
  useEffect(() => {
    const handleClickFora = (e) => {
      ...
      if (refinstrutor.current && !refinstrutor.current.contains(e.target)) {
        setMenuinstrutorAberto(false);
      }
      if (refadministrador.current && !refadministrador.current.contains(e.target)) {
