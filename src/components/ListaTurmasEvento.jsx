...
const TurmaCard = memo(function TurmaCard({
  ...
  const realizados = encontrosOrdenados.filter((d) => d <= hojeIso).length;
  ...
  const statusTurma = getStatusPorJanela({ di, df, hi, hf, agora: hojeDate });
...
export default function ListaTurmasEvento({
  ...
  const hojeIso = useMemo(() => ymdLocal(hoje), [hoje]);
  const [openMeetings, setOpenMeetings] = useState({});

  const handleToggleMeetings = useCallback((tid) => {
  ...
  return (
    <div id={`turmas-${eventoId}`} className="mt-5 space-y-5">
      {turmasOrdenadas.map((t) => (
        <TurmaCard
          key={t?.id || `${t?.nome || "Turma"}-${t?.data_inicio || ""}-${t?.horario_inicio || ""}`}
          t={t}
          hojeIso={hojeIso}
          hojeDate={hoje}
          ...
          jaInstrutorDoEvento={jaInstrutorDoEvento}
...
TurmaCard.propTypes = {
  ...
  hojeDate: PropTypes.instanceOf(Date).isRequired,
  ...
  openMeetings: PropTypes.object.isRequired,
  onToggleMeetings: PropTypes.func.isRequired,
  inscrever: PropTypes.func,
  inscrevendo: PropTypes.oneOfType([PropTypes.number, PropTypes.string]),
  jaInscritoNoEvento: PropTypes.bool.isRequired,
  jaInstrutorDoEvento: PropTypes.bool.isRequired,
