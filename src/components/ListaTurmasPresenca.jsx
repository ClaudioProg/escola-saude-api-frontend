...
const keyEventoMaisNovo = (ev) =>
  Math.max(...(ev?.turmas || []).map(keyFimTurma), -Infinity);
const ordenarEventosPorMaisNovo = (a, b) =>
  keyEventoMaisNovo(b) - keyEventoMaisNovo(a);
...
function exportarCSVDataAtiva(turmaId, dataYMD, mapaUsuarios) {
  ...
  const usuariosOrdenados = Array.from(mapaUsuarios.values()).sort((a, b) =>
  ...
  for (const u of usuariosOrdenados) {
    ...
    if (presente && prObj?.confirmadoEm) {
      ...
      if (
        !confirmadoStr &&
        typeof prObj.confirmadoEm === "string" &&
        /^\d{2}:\d{2}/.test(prObj.confirmadoEm)
      ) {
...
function StatPill({ icon: Icon, label, value, tone = "zinc" }) {
  const tones = {
    zinc: "bg-zinc-50 text-zinc-700 border-zinc-200 dark:bg-zinc-900/25 dark:text-zinc-200 dark:border-zinc-800",
    emerald:
      "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/25 dark:text-emerald-200 dark:border-emerald-800",
    rose: "bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-900/25 dark:text-rose-200 dark:border-rose-800",
    amber:
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/40 p-4">
      <div className="flex items-start gap-3">
        <AlertCircle className="w-5 h-5 text-zinc-500 mt-0.5" aria-hidden="true" />
...
export default function ListaTurmasPresenca({
  ...
  const [confirmPresenca, setConfirmPresenca] = useState(null);
  ...
  const carregarPresencas = useCallback(async (turmaId) => {
    ...
    try {
      ...
      const detalhes = await apiGet(`/api/presencas/turma/${turmaId}/detalhes`, {
        on403: "silent",
      });
  ...
  return (
    <>
                ...
                <div className="p-4 sm:p-6">
                  ...
                  <div className="mt-5 grid grid-cols-1 gap-4">
                    {turmasValidas.map((turma) => {
                      ...
                      const datasFallback =
                        inicioNoon && fimNoon
                          ? gerarIntervaloDeDatas(inicioNoon, fimNoon)
                          : [];
                      ...
                      const datasVisaoData = (presencasPorTurma[turma.id]?.datas || [])
                        .slice()
                        .sort();
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                              <div className="min-w-0">
                                ...
                                <div className="mt-3 flex flex-wrap gap-2">
                                  <StatPill
                                    icon={Users}
                                    label={`${(inscritosPorTurma?.[turma.id] || []).length} inscrito(s)`}
                                    tone="zinc"
                                  />
                                  <StatPill
                                    icon={CalendarDays}
                                    label={`${datasGrade.length} dia(s)`}
                                    tone="zinc"
                                  />
                                  <StatPill
                                    icon={BadgeCheck}
                                    label="Janela: -30min até +60 dias"
                                    tone="violet"
                                  />
                                </div>
                              </div>
                                  ...
                                  typeof gerarRelatorioPDF === "function" && (
                                    <button
                                      ...
                                      onClick={() =>
                                        gerarRelatorioPDF?.(
                                          turma.id,
                                          turma.nome || `turma_${turma.id}`
                                        )
                                      }
                                      className="inline-flex items-center gap-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 px-3 py-2 text-xs font-bold text-slate-900 dark:text-white"
                            ...
                            <AnimatePresence>
                              {modoadministradorPresencas && estaExpandida && (
                                ...
                                >
                                  {isLoadingTurma ? (
                                    <div
                                      className="animate-pulse rounded-2xl bg-zinc-100 dark:bg-zinc-800 h-28"
                                      aria-live="polite"
                                    />
                                  ) : (
                                      ...
                                      {agrupamento === "pessoa" && (
                                        <>
                                          <div className="flex items-center gap-2 mb-3">
                                            <ClipboardCheck className="w-4 h-4 text-teal-700 dark:text-teal-300" />
                                                ...
                                                .map((i) => {
                                                  ...
                                                  return (
                                                    <div
                                                      ...
                                                      className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950/30 overflow-hidden"
                                                    >
                                                      ...
                                                      <div className="px-4 pb-4 grid gap-2 md:hidden">
                                                        {datasGrade.map(({ dataISO, hi: hiDia }) => {
                                                          const info = Array.isArray(
                                                            usuarioBloco?.presencas
                                                          )
                                                            ? usuarioBloco.presencas.find(
                                                                (p) =>
                                                                  String(
                                                                    p?.usuario_id ??
                                                                      usuarioBloco.id
                                                          ...
                                                          const estaPresente = !!(
                                                            info && info?.presente === true
                                                          );
                                                          const presencaInfo = info
                                                            ? toPresencaInfo(info, dataISO)
                                                            : null;

                                                          let confirmadoStr = "—";
                                                          if (
                                                            estaPresente &&
                                                            presencaInfo?.confirmadoEm
                                                          ) {
                                                            confirmadoStr =
                                                              formatarDataHoraBR(
                                                                presencaInfo.confirmadoEm
                                                            ...
                                                            if (
                                                              confirmadoStr === "—" &&
                                                              typeof presencaInfo.confirmadoEm ===
                                                                "string" &&
                                                              /^\d{2}:\d{2}/.test(
                                                                presencaInfo.confirmadoEm
                                                              )
                                                            ) {
                                                              confirmadoStr = `${formatarDataBrasileira(
                                                                dataISO
                                                              )} ${presencaInfo.confirmadoEm.slice(
                                                                0,
                                                                5
                                                              )}`;
                                                            }
                                                          ...
                                                          const inicioAula =
                                                            toLocalDateFromYMDTime(
                                                              dataISO,
                                                              hiDia || hi
                                                            );
                                                          const abreJanela = addMinutes(
                                                            inicioAula,
                                                            -MINUTOS_ANTECIPACAO
                                                          );
                                                          const now = new Date();
                                                          ...
                                                          const dentroDaJanela =
                                                            abreJanela && janelaFinal
                                                              ? now >= abreJanela &&
                                                                now <= janelaFinal
                                                              : false;

                                                          const podeConfirmar =
                                                            !estaPresente && dentroDaJanela;
                                                            ...
                                                            >
                                                              <div className="flex items-start justify-between gap-2">
                                                                <div>
                                                                  ...
                                                                  <div className="text-xs text-zinc-600 dark:text-zinc-300">
                                                                    Início: {hiDia || hi} • Libera{" "}
                                                                    {MINUTOS_ANTECIPACAO}min antes
                                                                  </div>
                                                                </div>
                                                                <span
                                                                  className={cls(
                                                              ...
                                                              <div className="mt-2 flex items-center justify-between gap-2">
                                                                ...
                                                                {podeConfirmar ? (
                                                                  <button
                                                                    ...
                                                                    disabled={loading}
                                                                    className="rounded-xl px-3 py-2 text-xs font-extrabold text-white bg-teal-700 hover:bg-teal-800 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed"
                                                                  >
                                                                    {loading
                                                                      ? "Confirmando..."
                                                                      : "Confirmar"}
                                                                    ...
                                                                    <td className="py-2 px-3">
                                                                      {podeConfirmar ? (
                                                                        <button
                                                                          ...
                                                                          disabled={loading}
                                                                          className="rounded-xl px-3 py-2 text-[11px] font-extrabold text-white bg-teal-700 hover:bg-teal-800 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed"
                                                                        >
                                                                          {loading
                                                                            ? "Confirmando..."
                                                                            : "Confirmar"}
                                      ...
                                      {agrupamento === "data" && (
                                        <>
                                          {datasVisaoData.length === 0 ? (
                                            <EmptyState
                                              title="Nenhuma data registrada"
                                              text="Esta turma ainda não possui datas ou presenças carregadas."
                                            />
                                          ) : (
                                            (() => {
                                              const dataAtiva =
                                                dataAtivaPorTurma[turma.id] ||
                                                datasVisaoData[0];
                                              ...
                                              const dGrade = datasGrade.find(
                                                (x) => x.dataISO === dataAtiva
                                              );
                                              const hiDia = dGrade?.hi || hi;
                                              const hfDia = dGrade?.hf || hf;

                                              const inicioDia = toLocalDateFromYMDTime(
                                                dataAtiva,
                                                hiDia
                                              );
                                              const abreJanela = addMinutes(
                                              ...
                                              const soPend =
                                                !!somenteSemPresencaPorTurma[turma.id];
                                                ...
                                                <section className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white/70 dark:bg-zinc-900/40 overflow-hidden">
                                                      ...
                                                      <div>
                                                        ...
                                                        {antesDaJanela && (
                                                          <div className="text-[11px] text-amber-700 dark:text-amber-300 mt-1">
                                                            Confirmação manual libera{" "}
                                                            {MINUTOS_ANTECIPACAO} min antes do
                                                            início.
                                                          </div>
                                                        )}
                                                        {janelaFinal && (
                                                          <div className="text-[11px] text-zinc-500 dark:text-zinc-400 mt-1">
                                                            Janela final:{" "}
                                                            {formatarDataHoraBR(janelaFinal) ||
                                                              "—"}
                                                          </div>
                                                        )}
                                                      </div>
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-2">
                                                      <div className="min-w-[86px]">
                                                        <div className="inline-flex flex-col items-center justify-center px-3 py-2 rounded-2xl border border-zinc-200 bg-white shadow-sm dark:bg-zinc-900 dark:border-zinc-700">
                                                          <div className="leading-none font-extrabold text-2xl">
                                                            {totalInscritos}
                                                          </div>
                                                      ...
                                                      <div className="min-w-[86px]">
                                                        <div className="inline-flex flex-col items-center justify-center px-3 py-2 rounded-2xl border border-zinc-200 bg-white shadow-sm dark:bg-zinc-900 dark:border-zinc-700">
                                                          <div className="leading-none font-extrabold text-2xl text-emerald-600 dark:text-emerald-400">
                                                            {presentes}
                                                          </div>
                                                          <div className="text-[10px] uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                                                            presentes
                                                      ...
                                                      <div className="min-w-[86px]">
                                                        <div className="inline-flex flex-col items-center justify-center px-3 py-2 rounded-2xl border border-zinc-200 bg-white shadow-sm dark:bg-zinc-900 dark:border-zinc-700">
                                                          <div className="leading-none font-extrabold text-2xl text-rose-600 dark:text-rose-400">
                                                            {faltas}
                                                          </div>
                                                          <div className="text-[10px] uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                                                            faltas
                                                      ...
                                                      <div className="min-w-[86px]">
                                                        <div className="inline-flex flex-col items-center justify-center px-3 py-2 rounded-2xl border border-zinc-200 bg-white shadow-sm dark:bg-zinc-900 dark:border-zinc-700">
                                                          <div className="leading-none font-extrabold text-2xl text-amber-600 dark:text-amber-400">
                                                            {aguardando}
                                                          </div>
                                                          <div className="text-[10px] uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                                                            aguardando
                                                      ...
                                                      <button
                                                        ...
                                                        className="inline-flex items-center gap-2 bg-slate-200 hover:bg-slate-300 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-slate-900 dark:text-slate-100 text-xs px-3 py-2 rounded-xl"
                                                  ...
                                                  <div className="px-4 pb-2 flex items-center justify-between">
                                                    <label className="inline-flex items-center gap-2 text-xs text-zinc-700 dark:text-zinc-200">
                                                      <input
                                                        ...
                                                        onChange={(e) =>
                                                          setSomenteSemPresencaPorTurma((p) => ({
                                                            ...p,
                                                            [turma.id]: e.target.checked,
                                                          }))
                                                        }
                                                      />
                                                    ...
                                                    <div className="text-xs text-zinc-500">
                                                      {listaUsuarios.length} de {totalInscritos}{" "}
                                                      exibidos
                                                    </div>
                                                  </div>
                                                  ...
                                                  <div className="px-4 pb-4 grid gap-2 md:hidden">
                                                    {listaUsuarios.map((u) => {
                                                      ...
                                                      const abre = addMinutes(
                                                        inicioDiaDT,
                                                        -MINUTOS_ANTECIPACAO
                                                      );
                                                      const agora2 = new Date();
                                                      const podeConfirmar =
                                                        !presente && abre && janelaFinal
                                                          ? agora2 >= abre &&
                                                            agora2 <= janelaFinal
                                                          : false;

                                                      const statusTxt = presente
                                                        ? "Presente"
                                                        : abre && agora2 < abre
                                                        ? "Aguardando"
                                                        : "Faltou";
                                                          ...
                                                          <div className="mt-2 flex justify-end">
                                                            {podeConfirmar ? (
                                                              <button
                                                                ...
                                                                disabled={loading}
                                                                className="rounded-xl px-3 py-2 text-xs font-extrabold text-white bg-emerald-600 hover:bg-emerald-700 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed"
                                                              >
                                                                {loading
                                                                  ? "Confirmando..."
                                                                  : "Confirmar"}
                                                    ...
                                                    <table className="min-w-full text-sm table-fixed">
                                                      <thead>
                                                        <tr className="text-left text-zinc-600 dark:text-zinc-300">
                                                          ...
                                                          <th className="py-2 pr-4 w-[18%]">
                                                            Situação
                                                          </th>
                                                          <th className="py-2 pr-4 w-[22%]">
                                                            Confirmado em
                                                          </th>
                                                              ...
                                                              <td className="py-2 pr-4">
                                                                {podeConfirmar ? (
                                                                  <button
                                                                    ...
                                                                    disabled={loading}
                                                                    className="inline-flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-extrabold text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 disabled:cursor-not-allowed"
                                                                  >
                                                                    {loading
                                                                      ? "Confirmando..."
                                                                      : "Confirmar"}
      ...
      <ModalConfirmacao
        ...
        onConfirmar={() =>
          confirmPresenca ? confirmarPresencaAgora(confirmPresenca) : true
        }
          ...
          confirmPresenca ? (
            <div className="space-y-2">
              <p>
                ...
                <strong>{confirmPresenca.nome || "participante"}</strong> em{" "}
      ...
      <ModalConfirmacao
        ...
        onConfirmar={() =>
          confirmRemover ? removerTurmaAgora(confirmRemover) : true
        }
          ...
          confirmRemover ? (
            <div className="space-y-2">
              <p>
                Remover a turma{" "}
                <strong>
                  {confirmRemover.turmaNome || `#${confirmRemover.turmaId}`}
                </strong>
                ?
              </p>
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-900/40 dark:bg-amber-900/20 dark:text-amber-200">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p>
                      Se houver <strong>presenças</strong> ou{" "}
                      <strong>certificados</strong>, o backend bloqueará a exclusão.
                    </p>
                  </div>
                </div>
              </div>
