...
export default function ModalTurma({
  ...
  // ref para auto-size do título
  const refNome = useRef(null);
  const autosizeNome = () => {
    const el = refNome.current;
    if (!el) return;
    el.style.height = "auto";
    el.style.height = `${Math.min(el.scrollHeight, 200)}px`; // trava máx 200px
  };
  ...
  const validar = () => {
    ...
    if (nome.length > NOME_TURMA_MAX) {
      console.warn("❌ Nome excede o limite:", nome.length);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col max-h-[90vh] bg-white dark:bg-zinc-900">
        ...
        >
          {/* Nome — agora textarea com contador/limite e auto-resize */}
